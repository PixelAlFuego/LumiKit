...
## Archivos
- `Assets/LumiKit/Fonts/` (existe) · cuatro `TMP_FontAsset` (existen) con los nombres literales del GDD líneas 932-936: `SpaceGrotesk-Medium SDF.asset`, `Inter-Regular SDF.asset`, `Inter-Medium SDF.asset`, `JetBrainsMono-Regular SDF.asset`. **Los genera el usuario.**
- `Assets/LumiKit/Fonts/Licenses/` (existe) · `OFL_SpaceGrotesk.txt`, `OFL_Inter.txt`, `OFL_JetBrainsMono.txt`. El GDD dibuja un único `OFL.txt`; son tres porque cada familia trae su línea de copyright y esa línea no se fusiona ni se reescribe.
- `Assets/LumiKit/Fonts/Source/` (existe) · los `.ttf` originales (`SpaceGrotesk-Medium`, `Inter_18pt-Regular`, `Inter_18pt-Medium`, `JetBrainsMono-Regular`), que **sí entran en el pack**: desviación del árbol del GDD decidida por el usuario y anotada en **D-009**. No pesan en el build, sólo en el `.unitypackage`.
- `Assets/Editor/ParameterPanelBuilder.cs` (existe) · **no aditiva**: `CreateText` gana un parámetro de familia y sus seis llamadas lo pasan (las cuatro etiquetas de fila entran por `CreateRowLabel`). Nueve líneas ya escritas: una por llamada, la firma y el comentario de `HasDefaultFont`.
...
- `.claude/rules/ui-style.md` (existe) · una frase de Tipografía pasa a apuntar a `Assets/LumiKit/Fonts/`, sin subir de las 60 líneas en que LK-22a la deja. Regenera (D-002): los seis `PRF_*` de `Prefabs/UI/`; cambian todos, porque todos llevan texto.
- No se tocan: `Core/`, `Utils/`, `Demo/`, los scripts de `UI/` (salvo el comentario de `LumiTheme` sobre fuentes), `Assets/TextMesh Pro/` ni `TMP Settings.asset` — la fuente por defecto del proyecto se queda como está; el pack no la impone.
...
2. `RequireFonts()` al principio de `GeneratePrefabs`, junto a `HasDefaultFont()`: si falta alguna de las tres que se usan, error **nombrando las rutas que faltan**, diálogo remitiendo a esta spec, y aborta sin escribir nada.
...
`Inter-Regular` se genera pero **hoy no lo usa ningún texto del panel**: es el nivel Body del GDD (línea 455) y entra con descripciones y toasts (LK-25, LK-31, LK-34). Se genera ahora para no volver a abrir el Font Asset Creator. **El generador no la exige** (usuario, Sesión 10; mismo criterio que `SPR_UI_Ring`): exige sólo lo que usa. Su ruta queda como constante y entra en `RequireFonts()` con la familia Body (LK-25).
...
Por eso, **antes de generar, mirar la primera línea de cada `OFL_*.txt`**: si dice `with Reserved Font Name`, ese `.asset` se renombra sin el nombre de la familia; si no, se deja el del GDD. Lo comprueba el usuario. Resumen por familia → LK-26.
...
| Paso | Qué | Dónde |
|---|---|---|
| 1 | Descargar los `.ttf` **estáticos**, no los variables, y dejarlos en `Fonts/Source/` (D-009): `SpaceGrotesk-Medium`, `Inter_18pt-Regular`, `Inter_18pt-Medium`, `JetBrainsMono-Regular`. Google Fonts entrega estáticos; los repos de Inter y Space Grotesk entregan además variables, que el Font Asset Creator no hornea bien | fuera de Unity |
| 2 | Guardar la licencia de cada familia como `OFL_<Familia>.txt` y mirar su línea de copyright | `Fonts/Licenses/` |
...
| Padding | **7 px**, con el selector en `px` y no en `%` (en % el creador da `(int)(70 × 7 / 100)` = 4) — ratio del 10 %, el mismo en las cuatro |
...
| Render Mode | **SDFAA** · Get Font Features ✔ |
...
- [ ] Ejecutar el menú de generación con una de las tres fuentes que se usan fuera de `Fonts/`: error nombrando la ruta que falta y **ningún prefab escrito** (`git status` limpio en `Prefabs/UI/`).
...
- [ ] Los valores en JetBrains Mono y las etiquetas en Inter, ambos a 16, se ven de tamaño equivalente. Si no, se corrige con un token en `LumiTheme`, nunca a mano.
...
- [ ] Tras salir de Play, `git status` no muestra cambios en `MAT_Debug.mat` (D-001) ni en `LiberationSans SDF - Fallback.asset`.
...
- Tocar `Core`, `Demo`, los scripts de `UI/` (salvo el comentario de `LumiTheme`), `TMP Settings.asset` o el `EffectDebugTester`.
